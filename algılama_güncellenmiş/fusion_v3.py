...
class DebugFusionSystem:
    def __init__(self):
        ...
        self.red_buoy_pub   = rospy.Publisher('/fusion_output/red_buoy',   Float32MultiArray, queue_size=10)
        self.green_buoy_pub = rospy.Publisher('/fusion_output/green_buoy', Float32MultiArray, queue_size=10)
    ...
    def perform_fusion(self):
        ...
        try:
            ...
            if obstacles:
                ...
                # Publish color-separated buoy arrays
                red_arr = Float32MultiArray()
                green_arr = Float32MultiArray()
                for obs in obstacles:
                    label = str(obs.get('class','')).lower()
                    pair = [float(obs['angle']), float(obs['distance'])]
                    if 'red' in label:
                        red_arr.data += pair
                    elif 'green' in label:
                        green_arr.data += pair
                if red_arr.data:
                    self.red_buoy_pub.publish(red_arr)
                    rospy.loginfo(f"[FUSION] Published to /fusion_output/red_buoy: {red_arr.data}")
                if green_arr.data:
                    self.green_buoy_pub.publish(green_arr)
                    rospy.loginfo(f"[FUSION] Published to /fusion_output/green_buoy: {green_arr.data}")

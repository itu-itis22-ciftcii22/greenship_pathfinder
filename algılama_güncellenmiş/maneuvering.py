            ...
            while not rospy.is_shutdown():
                try:
                    ...
                    for mission_idx, mission_ned in enumerate(missions_ned):
                        ...
                        # ========== BU KISMI EKLE - SATIR 244'E ==========
                        # 10. waypoint'ten önce dubalara bakma
                        if mission_idx < 9:  # 10. waypoint index 9
                            rospy.loginfo("Normal navigasyon - Dubalar dikkate alınmıyor")
                            
                            # Direkt waypoint'e git
                            target_lat, target_lon = ned_to_global_scaled(
                                home_global[0], home_global[1],
                                mission_coord.row-75, 
                                mission_coord.col-75
                            )
                            vehicle.assignWPs([(target_lat, target_lon)])
                            ...
                            while not rospy.is_shutdown():
                                ...
                                if msg is not None:
                                    ...
                                    if distance_to_mission <= mission_radius:
                                        ...
                                        break
                                rate.sleep()
                            continue  # Sonraki waypoint'e geç
                        
                        # 10. waypoint ve sonrası - dubalı görev
                        rospy.loginfo("DUBALI GÖREV AKTİF!")
                        # ========== EKLENECEK KOD BİTTİ ==========

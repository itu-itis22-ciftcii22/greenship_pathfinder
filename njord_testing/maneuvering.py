...
class CommanderManeuver(Commander):
    def __init__(self, rate, domain, vehicle):
        ...
        rospy.loginfo("Maneuvering commander initialized with fusion support")
    
    def fusion_callback(self, msg):
        """Process fusion obstacle data"""
        ...
        rospy.logdebug(f"Fusion callback: received {len(self.fusion_obstacles)} obstacles")

    def execute_task_code_maneuver(self):
        ...
        for i, target_ned in enumerate(self.mission_ned_list):
            ...
            while not reached and not rospy.is_shutdown():
                ...
                try:
                    ...
                    # Use fusion data if available and fresh
                    if (self.last_fusion_time and 
                        ...
                        rospy.logdebug(f"Using {len(dynamic_obstacle_coords)} fusion obstacles")
                    else:
                        # No fusion data available - use basic navigation
                        dynamic_obstacle_coords = []
                        rospy.logwarn_throttle(5.0, "No fresh fusion data, proceeding with basic navigation")

...
- GPS logging for start/end coordinates (navigation still vision-based)
...
class MavlinkClient:
   ...
   def get_gps_position(self, timeout=1.0):
       """GPS pozisyonunu oku (opsiyonel, sadece loglama için)"""
       msg = self.master.recv_match(type='GLOBAL_POSITION_INT', blocking=True, timeout=timeout)
       if msg:
           lat = msg.lat / 1e7  # Convert from int
           lon = msg.lon / 1e7
...
class Navigator:
   def __init__(self, args):
       ...
       # GPS koordinatları (sadece loglama için)
       self.start_coords = (args.start_lat, args.start_lon) if args.start_lat else None
       self.end_coords = (args.end_lat, args.end_lon) if args.end_lat else None
       self.log_gps = args.log_gps
       ...
       if self.start_coords:
           print(f"[GPS] Start: {self.start_coords[0]:.6f}, {self.start_coords[1]:.6f}")
       if self.end_coords:
           print(f"[GPS] Target End: {self.end_coords[0]:.6f}, {self.end_coords[1]:.6f}")
   ...
   def loop(self):
       last_send = time.time()
       gps_log_interval = 1.0  # Her 1 saniyede bir logla
       last_gps_log = time.time()
       ...
       while True:
           ...
           # GPS Loglama (navigasyonu etkilemez)
           if self.log_gps and (time.time() - last_gps_log > gps_log_interval):
               gps_pos = self.mav.get_gps_position()
               if gps_pos:
                   ...
                   # Hedefe uzaklık hesapla (sadece bilgi için)
                   if self.end_coords:
                       dist = haversine_distance(gps_pos[0], gps_pos[1], 
                                               self.end_coords[0], self.end_coords[1])
                       print(f"[GPS] Distance to end: {dist:.1f}m")
                       
                       # Bitiş noktasına yaklaşıldı mı?
                       if dist < 5.0:  # 5 metre yakınlık
                           print(f"[SUCCESS] Reached end point! Distance: {dist:.1f}m")
                           # GPS logunu kaydet
                           with open("gps_track.txt", "w") as f:
                               f.write(f"Start: {self.start_coords}\n")
                               f.write(f"End: {self.end_coords}\n")
                               f.write(f"GPS Log: {self.gps_log}\n")
...
def parse_args():
   ...
   # GPS koordinat argümanları
   ap.add_argument("--start-lat", type=float, default=41.025600, help="Start latitude (Teknofest 2024)")
   ap.add_argument("--start-lon", type=float, default=28.889200, help="Start longitude (Teknofest 2024)")
   ap.add_argument("--end-lat", type=float, default=41.027800, help="End latitude (Teknofest 2024)")
   ap.add_argument("--end-lon", type=float, default=28.891400, help="End longitude (Teknofest 2024)")
   ap.add_argument("--log-gps", action="store_true", default=True, help="Log GPS position during run")
